...
additionalFiles = [string(existingAdditionalFiles), string(authResources), ...
                   string(authProfileAvatar), string(uiDownloadAvatar)];
compiler.build.standaloneApplication(appFile, ...
...
No Application Compiler, adicione `authResources`, `authProfileAvatar` e `uiDownloadAvatar` em **Files installed for your end user**. A pasta de recursos deve conter os dois arquivos abaixo no pacote final:
