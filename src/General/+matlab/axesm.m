function axesm(varargin)
%AXESM Create axesm-based map

% Copyright 1996-2023 The MathWorks, Inc.
% Edited by Eric Magalhães Delgado (Aug-6 2025)

mstruct = map.internal.initmstruct; %  AXESM algorithm requires mfields to
...
mstruct = map.internal.resetmstruct(mstruct);
...
if resetFrameGrat
    ...
    set(gca(axesHandle.Parent), 'UserData', mstruct);
else
    % Set GCA to be a axesm-based map
    newplot(axesHandle)
    setgca(mstruct, axesHandle)
end
...
switch propname
    ...
    case 'zone'
        mstruct = map.internal.setZone(mstruct, propvalu);

    case 'origin'
        mstruct = map.internal.setOrigin(mstruct, propvalu);
    ...
    case 'maplatlimit'
        if isempty(propvalu)
            ...
            mstruct = map.internal.setMapLatLimit(mstruct, propvalu);
        end
    ...
    case 'maplonlimit'
        if isempty(propvalu)
            ...
            mstruct = map.internal.setMapLonLimit(mstruct, propvalu);
        end
...
function setgca(mstruct, axesHandle)

%  Set GCA to be axesm-based map.
set(axesHandle, ...
    'NextPlot','Add',...
    'UserData',mstruct,...
