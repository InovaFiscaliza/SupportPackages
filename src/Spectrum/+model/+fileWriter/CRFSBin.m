function CRFSBin(fileName, specData)
    ...
    Write_DataType21(fileID, specData(1))
    
    % GPS data (DataType 40)
    if specData(1).GPS.Status
        Write_DataType40(fileID, specData(1))
    end
    ...
    for ii = 1:numel(specData)
        nSweep = numel(specData(ii).Data{1});
        
        switch specData(ii).MetaData.Antenna.SwitchMode
            case 'manual'; antennaPort = specData(ii).MetaData.Antenna.Port;
            case 'auto';   antennaPort = 0;
        end
        
        if ismember(specData(ii).MetaData.DataType, SPECTYPES)
            for jj = 1:nSweep
                Write_DataType67(fileID, specData(ii), antennaPort, jj)
            end
            
        elseif ismember(specData(ii).MetaData.DataType, OCCTYPES)
            for jj = 1:nSweep
                Write_DataType69(fileID, specData(ii), antennaPort, jj)
            end
        end
    end
...
%-------------------------------------------------------------------------%
function Write_DataType21(fileID, specData)
    
    Hostname = specData.Receiver;
    ...
    method = specData.RelatedFiles.Task{1};
...
%-------------------------------------------------------------------------%
function Write_DataType40(fileID, specData)
    
    Write_BlockHeader(fileID, 1, 40, 40);
    
    fwrite(fileID,   day(specData.Data{1}(1)));                             % Wall Clock Date
    fwrite(fileID, month(specData.Data{1}(1)));
    fwrite(fileID,  year(specData.Data{1}(1))-2000);
    fwrite(fileID, 0);
    
    fwrite(fileID,   hour(specData.Data{1}(1)));                            % Wall Clock Time
    fwrite(fileID, minute(specData.Data{1}(1)));
    fwrite(fileID, second(specData.Data{1}(1)));
    fwrite(fileID, 0);
    ...
    fwrite(fileID, specData.GPS.Status);                                    % Positional fix and status of first GPS block
    fwrite(fileID, 0);                                                      % Sattelites
    fwrite(fileID, 0, 'uint16');                                            % Heading
    fwrite(fileID, specData.GPS.Latitude  .* 1e+6, 'int32');                % Latitude   (mean)
    fwrite(fileID, specData.GPS.Longitude .* 1e+6, 'int32');                % Longitude  (mean)
...
%-------------------------------------------------------------------------%
function Write_DataType67(fileID, specData, antennaPort, idx)
    
    % Description
    Description = specData.RelatedFiles.Description{1};
    ...
    % NPAD and NDATA
    NPAD  = 0;
    NDATA = specData.MetaData.DataPoints;
    ...
    Write_BlockHeader(fileID, specData.RelatedFiles.ID(1), BytesBlock, 67);
    
    fwrite(fileID,   day(specData.Data{1}(idx)));                           % WALLDATE
    fwrite(fileID, month(specData.Data{1}(idx)));
    fwrite(fileID,  year(specData.Data{1}(idx))-2000);
    fwrite(fileID, 0);
    
    fwrite(fileID,   hour(specData.Data{1}(idx)));                          % WALLTIME
    fwrite(fileID, minute(specData.Data{1}(idx)));
    fwrite(fileID, second(specData.Data{1}(idx)));
    fwrite(fileID, 0);
    fwrite(fileID, (second(specData.Data{1}(idx))-fix(second(specData.Data{1}(idx))))*1e+9, 'uint32');
    ...
    F0 = specData.MetaData.FreqStart ./ 1e+6;                               % FreqStart in MHz
    IntegerPart = fix(F0);
    DecimalPart = round((F0 - IntegerPart).*1e+3);
    fwrite(fileID, IntegerPart, 'uint16');
    fwrite(fileID, DecimalPart, 'int32');
    
    F1 = specData.MetaData.FreqStop ./ 1e+6;                                % FreqStop in MHz
    IntegerPart = fix(F1);
    DecimalPart = round((F1 - IntegerPart).*1e+3);
    fwrite(fileID, IntegerPart, 'uint16');
    fwrite(fileID, DecimalPart, 'int32');
    
    if ~isempty(specData.MetaData.Resolution)                               % Resolution in Hz
        fwrite(fileID, specData.MetaData.Resolution, 'int32');
    else
    ...
    switch specData.MetaData.TraceMode
    ...
    switch specData.MetaData.LevelUnit                                      % Data Type (0: dBm; 1: dBµV/m)
        case 'dBm';    levelID = 0;
        case 'dBµV/m'; levelID = 1;
    end
    fwrite(fileID, levelID);
    
    OFFSET = ceil(max(specData.Data{2}(:,idx)));
    if OFFSET < -20
    ...
    EncodedSpec = 2.*(specData.Data{2}(:,idx) - OFFSET) + 255;    
...
%-------------------------------------------------------------------------%
function Write_DataType69(fileID, specData, antennaPort, idx)
    ...
    Write_BlockHeader(fileID, specData.RelatedFiles.ID(1), BytesBlock, 69);
    ...
    fwrite(fileID, NPAD);                                                   % NPAD (Number of bytes of padding)
    fwrite(fileID, specData.MetaData.Threshold, 'int16');                   % Threshold
    fwrite(fileID, specData.RelatedFiles.RevisitTime(1), 'uint16');         % Integration time
    
    fwrite(fileID, NDATA, 'uint32');                                        % NDATA
    
    EncodedSpec = 2.*specData.Data{2}(:,idx);
    fwrite(fileID, EncodedSpec, 'uint8');
    fwrite(fileID, zeros(1,NPAD));                                          % Padding
